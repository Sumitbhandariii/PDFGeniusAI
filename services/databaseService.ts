/**
 * Database Service - OnSpace Cloud (Supabase) operations
...
export async function fetchCloudPDFFiles(): Promise<{
  data: CloudPDFFile[];
  error: string | null;
}> {
  try {
...
export async function fetchBookmarks(pdfId: string): Promise<{
  data: Bookmark[];
  error: string | null;
}> {
  try {
...
export async function fetchAIChats(): Promise<{
  data: AIChat[];
  error: string | null;
}> {
  try {
...
export async function fetchAIMessages(chatId: string): Promise<{
  data: AIMessage[];
  error: string | null;
}> {
  try {
...
// Helper: decode base64 string to Uint8Array for storage upload
