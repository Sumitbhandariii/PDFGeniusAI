 ...
 * Web-compatible version without expo-file-system
 */
...
export async function syncPDFToCloud(file: PDFFile): Promise<{ error: string | null }> {
  try {
    ...
    // On web, upload PDF from data URI
    let uploadError = null;
    const storagePath = `${userData.user.id}/${file.id}/${file.name}`;

    if (file.localUri && file.localUri.startsWith('data:')) {
      const base64 = file.localUri.split(',')[1];
      if (base64) {
